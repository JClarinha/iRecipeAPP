...
@Component({
  selector: 'app-rejected-recipes',
  standalone: true,
  imports: [CommonModule, FontAwesomeModule, ReactiveFormsModule, FormsModule],
  templateUrl: './rejected-recipes.component.html',
  styleUrl: './rejected-recipes.component.scss'
})
export class RejectedRecipesComponent {
  ...
  getAll() {
    this.recipeService.getAll().subscribe((response: Recipe[]) => {
        this.recipes = response.filter(recipe => recipe.approval === false);
